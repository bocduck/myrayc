...
func handle(c net.Conn, si *serverInfo) {
	...
	go func() {
		...
		var b bytes.Buffer
		b.Grow(4096)
		writeHTTPUpgradeRequest(&b, si)
		writeVLESSRequest(&b, host, port)
		//b_debug := b.Len()
		buf := b.AvailableBuffer()
		n, err := c.Read(buf[:cap(buf)])
		if n > 0 {
			b.Write(buf[:n])
			//fmt.Printf("b_debug: %d, %d, %d\n", b_debug, n, b.Len())
			_, errW := remote.Write(b.Bytes())
			if errW != nil {
				errCh <- errW
				return
			}
...
func writeHTTPUpgradeRequest(w io.Writer, si *serverInfo) error {
	req := &http.Request{
		Method: "GET",
		URL: &url.URL{
			Scheme: "http",
