...
func main() {
	...
	fmt.Println(serverAddr, serverName, serverPath, enableTLS, serverHost)
	//panic("test")
...
func handleSOCKS(c net.Conn) {
	...
	host, port, err := socks5ReadRequest(br)
	if err != nil {
		return
	}
...
func socks5ReadRequest(br *bufio.Reader) (string, uint16, error) {
	h := make([]byte, 4)
	if _, err := io.ReadFull(br, h); err != nil {
		return "", 0, err
	}
