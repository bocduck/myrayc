...
func handle(c net.Conn, si *serverInfo) {
	...
	remote, err := dialVLESS(si, host, port)
	if err != nil {
	...
	go func() {
		//Unwrap here but not in dialVLESS() for "0-rtt"
...
func dialVLESS(si *serverInfo, host string, port uint16) (net.Conn, error) {
	...
	err = writeHTTPUpgradeRequest(conn, si)
	if err != nil {
		return nil, err
	}

	err = writeVLESSRequest(conn, host, port)
	if err != nil {
		return nil, err
	}
...
func unwrapVLESS(br *bufio.Reader) error {
...
func writeHTTPUpgradeRequest(conn net.Conn, si *serverInfo) error {
	req := &http.Request{
		Method: "GET",
		URL: &url.URL{
			Scheme: "http",
